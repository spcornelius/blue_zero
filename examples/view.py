...
untrained_model_save_file = Path(f"./saved_models/untrained_model_20.pt")
trained_model_save_file = Path(f"./saved_models/trained_model_20.pt")
...
smart_net = QNet(hp.num_feat, hp.num_hidden, hp.depth,
                 kernel_size=hp.kernel_size)
smart_net.load_state_dict(torch.load(trained_model_save_file))
smart_net.eval()
smart_agent = Agent(smart_net)

dumb_net = QNet(hp.num_feat, hp.num_hidden, hp.depth,
                 kernel_size=hp.kernel_size)
